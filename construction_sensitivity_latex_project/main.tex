...
\title{Construction Sensitivity of a Shortest-Path Anisotropy Diagnostic\\
...
\begin{abstract}
Geometric-graph observables can depend on sampling, edge construction, and the
choice of control. We examine this dependence for a shell-based shortest-path
anisotropy statistic, $\clog$, on graphs sampled from Schwarzschild/Flamm,
Bardeen, and Hayward embedding surfaces. Canonical ten-seed Bardeen and Hayward
parameter scans show organized but realization-dependent radial behavior.
Paired audits at Schwarzschild/Flamm, Bardeen $g=0.2$, and Hayward $\ell=0.2$
compare five graph/control protocols using the same radial-angular sample and
twelve common radial bins.

For Schwarzschild, mean absolute black-hole/control correlation gaps are
$0.242\pm0.161$, $0.688\pm0.442$, $0.233\pm0.199$,
$0.422\pm0.323$, and $0.681\pm0.445$ for the original, strict-radius,
pure-KNN, mutual-KNN, and edge-matched-radius protocols (mean $\pm$ standard
deviation over ten seeds). The Bardeen and Hayward anchors show the same
qualitative construction dependence: original gaps are $0.255\pm0.174$ and
$0.250\pm0.165$, whereas edge-matched-radius gaps are $0.676\pm0.452$ and
$0.680\pm0.443$. Radius-style controls therefore do not erase the separation,
and signed gaps vary across realizations and protocols.

The black-hole/control separation is neither construction invariant nor
realization independent. $\clog$ is reproducible when the complete sampling,
construction, binning, seed, and null-model protocol is specified, but the
evidence does not support interpreting it as a graph-independent curvature
scalar. The contribution is a matched-control audit framework for attaching
geometric claims to a complete diagnostic--construction--control protocol.
\end{abstract}

\noindent\textbf{Keywords:} geometric graphs; spatial networks; shortest-path multiplicity;
graph construction; matched controls; black-hole embeddings
...
An earlier version of this work reported fixed-protocol and
single-realization results \cite{PalominoYlla2026preprint}. The present
manuscript replaces that construction analysis with paired ten-seed audits,
adds canonical Bardeen and Hayward calculations, and removes quantitative
claims that did not survive the multi-seed validation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/black_hole_family_colored_by_LogCMD_3models_smoothed.pdf}
    ...
    \par\small\textit{Alt text:} Three colored three-dimensional geometric
    graphs show Schwarzschild/Flamm, Bardeen, and Hayward embedding surfaces.
    A shared color scale reveals comparable radial organization across the
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.94\linewidth]{figures/black_hole_family_radial_profile_plot3.pdf}
    ...
    \par\small\textit{Alt text:} Binned mean $C_{\log}$ is plotted against
    radial coordinate for Schwarzschild/Flamm, Bardeen $g=0.2$, and Hayward
    $\ell=0.2$. The three profiles and error bars closely track one another.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/audit_mean_abs_gap.pdf}
    ...
    substantial.}
    \par\small\textit{Alt text:} Mean absolute black-hole/control correlation
    gap with standard-deviation error bars for five graph protocols. The two
    radius-based controls have the largest means and broad variation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{figures/audit_paired_abs_gap.pdf}
    \caption{Paired $|\drho|$ values for all ten seeds. Lines join the same
    radial-angular sample across graph/control protocols.}
    \par\small\textit{Alt text:} Ten colored seed trajectories show absolute
    correlation gap across five categorical protocols. The ordering changes by
    seed, with frequent peaks at the radius-based controls.
    \label{fig:audit-paired-abs}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\linewidth]{figures/audit_paired_signed_gap.pdf}
    ...
    \par\small\textit{Alt text:} Ten paired seed trajectories show signed
    correlation gap across five protocols. Several trajectories cross zero,
    especially for original, pure-KNN, and mutual-KNN graphs.
...
\begin{figure}[H]
\centering
\includegraphics[width=.49\linewidth]{figures/bardeen_canonical_parameter_scan.pdf}
\includegraphics[width=.49\linewidth]{figures/hayward_canonical_parameter_scan.pdf}
\caption{Canonical Bardeen (left) and Hayward (right) parameter scans. Error
bars show sample SD across paired seeds.}
\par\small\textit{Alt text:} Side-by-side Bardeen $g$ and Hayward $\ell$
parameter scans compare black-hole radial correlation, flat-control
correlation, and absolute gap. Parameter shifts are modest relative to the
...
\begin{figure}[H]
\centering
\includegraphics[width=.49\linewidth]{figures/bardeen_anchor_mean_abs_gap.pdf}
\includegraphics[width=.49\linewidth]{figures/hayward_anchor_mean_abs_gap.pdf}
...
\par\small\textit{Alt text:} Side-by-side five-protocol error-bar plots for
Bardeen $g=0.2$ and Hayward $\ell=0.2$. Both show smaller original and pure-KNN
gaps and larger radius-control gaps with broad uncertainty.
...
\bibitem{PalominoYlla2026preprint}
A. U. Palomino Ylla,
Construction sensitivity of shortest-path anisotropy diagnostics in geometric
graphs from black-hole embeddings,
\emph{arXiv preprint} arXiv:2606.27248 (2026).
