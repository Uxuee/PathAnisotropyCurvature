...
The project began as an exploratory Wolfram Community / Wolfram Summer School idea about whether a Kretschmann-like scalar could be defined for hypergraphs or graph-based discrete geometries. The current repository tests whether local shortest-path anisotropy contains curvature-organized information in graph structures built from continuum curvature benchmarks.

The goal is **not** to claim that the estimator is a discrete Kretschmann scalar. Instead, the goal is to test whether local shortest-path anisotropy contains curvature-organized information in graph structures built from continuum curvature benchmarks.
...
> The logarithmic path-anisotropy estimator provides a local graph observable whose radial organization is strongly associated with black-hole curvature profiles under a fixed graph-construction and matched-flat controls.
...
The estimator is defined on graph-distance shells. For each reference vertex $p$, we count the number of distinct shortest paths from $p$ to vertices $q$ on a graph-distance shell $S_{r_g}(p)$, and measure the statistical heterogeneity of these counts.

The strongest current results are obtained for embedded black-hole spatial slices. In calibrated graph discretizations of Schwarzschild/Flamm, Reissner–Nordström, Bardeen, and Hayward geometries, the estimator shows a strong correlation with radial coordinate and negative correlation with the Kretschmann scalar.
...
The estimator measures how uneven these shortest-path counts are across the shell. The working hypothesis is that curvature-organized graph structure leaves a statistical signature in local shortest-path anisotropy.

The current main estimator is the logarithmic cubic-mean-deviation version:

$$C_{\log}(p,r_g) = \left[\frac{1}{|S_{r_g}(p)|}\sum_{q\in S_{r_g}(p)}\left|\log N_{\mathrm{geo}}(p,q)-\overline{\log N}_{p,r_g}\right|^3\right]^{1/3}.$$
...
The negative values of $\mathrm{Corr}(\log K,C_{\log})$ in this fixed-parameter table reflect the fact that, in the binned-profile convention used for the cross-family plot, $C_{\log}$ increases outward while $K$ decreases outward.
...
This keeps radial sampling fixed while removing the black-hole embedding height. The matched-flat control is therefore a stronger null test than a generic flat disk, because it controls for radial sampling bias.
...
For matched-flat controls, graph-radius sensitivity experiments, Forman-Ricci baselines, Ollivier-Ricci baselines, and black-hole-family scans, see the helper functions in `code/CurvatureEstimator.wl`.
...
This is an exploratory research repository. The current evidence suggests that shortest-path anisotropy tracks the spatial organization of curvature-sensitive graph structure better than raw pointwise degree statistics or standard transport-based baselines.
...
The original project asked whether a curvature scalar analogous to the Kretschmann scalar could be defined for hypergraph-based discrete geometries. The present repository reformulates that idea as a shortest-path-anisotropy diagnostic on graph discretizations of continuum geometries.
